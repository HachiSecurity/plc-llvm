...
withCurried name ps@((sn,isTyVar):dyn) builder = do
    ...
    let mkCurry :: Int -> S.Set T.Text -> [(T.Text,Bool)]
        ...
        mkCurry ix fvs ((vn,isTyVar'):vs) = do
            ...
            compileDynamicClosure isTyVar' dynName fvs vn $ \_ arg -> do
...
fstPair =
    let ps = [(closureTyPtr, True), (closureTyPtr, True), (pairTyPtr, False)]
    in compileCurried "fstPair" ps $ \[_,_,ptr] -> getFst ptr

sndPair :: MonadCodeGen m => m ClosurePtr
sndPair =
    let ps = [(closureTyPtr, True), (closureTyPtr, True), (pairTyPtr, False)]
    in compileCurried "sndPair" ps $ \[_,_,ptr] -> getSnd ptr
